...
public class DialogueSelector : MonoBehaviour
{
    ...
    {
        (DialogueData main, _) = SelectInternal(npcId);
        return main;
    }
    ...
    {
        (_, IReadOnlyList<DialogueData> questList) = SelectInternal(npcId);
    ...
    private (DialogueData main, IReadOnlyList<DialogueData> questList) SelectInternal(string npcId)
    {
        ...
        IReadOnlyList<DialogueData> candidates = dm.GetDialoguesForNpc(npcId);
        if (candidates == null || candidates.Count == 0) return (null, null);

        Func<string, int> getFlag = key => fm.GetFlag(key);
        List<DialogueData> matching = candidates.Where(d => d != null && d.MatchesFlags(getFlag)).OrderBy(d => d.priority).ToList();

        DialogueData firstTalk = matching.FirstOrDefault(d => d.category == DialogueCategory.FirstTalk);
        if (firstTalk != null) return (firstTalk, null);

        DialogueData casual = matching.FirstOrDefault(d => d.category == DialogueCategory.Casual);
        List<DialogueData> questList = matching.Where(d => d.category == DialogueCategory.Quest).ToList();
